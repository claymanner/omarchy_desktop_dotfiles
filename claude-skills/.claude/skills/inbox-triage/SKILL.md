...
- **Never triage emails from `pile@e-wee.com`** (pile's own digest/reminder emails). If the classifier sees these, it'll recursively create pile items for tasks pile is already tracking — infinite-loop noise. Auto-archive without classification. Same rule applies to any future self-addressed automation emails (e.g. cron-job summaries, internal-system digests).
